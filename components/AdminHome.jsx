'use client'

import Link from 'next/link';
import DeleteButton from '@/components/buttons/DeleteButton';
import Image from 'next/image';
import { useEffect, useState } from 'react';


export default function AdminHome({ session }) {
  const [articles, setArticles] = useState([]);

  useEffect(() => {
    async function fetchArticles() {
      const res = await fetch('/api/admin/articles');
      const data = await res.json();
      setArticles(data);
    }

    fetchArticles();
  }, []);


  return (
    <div className="admin-dashboard">
      <header className="admin-dashboard__header">
        <div className="admin-dashboard__headerText">
          <h1 className="admin-dashboard__title">Espace Admin</h1>
          <p className="admin-dashboard__subtitle">Bienvenue {session?.user?.name}</p>
        </div>

        <Link className="admin-dashboard__newButton" href="/admin/new">
          ➕ Nouvel article
        </Link>
      </header>

      <h2 className="admin-dashboard__sectionTitle">Articles existants</h2>

      <ul className="admin-dashboard__list">
        {articles.map((article) => (
          <li key={article.id} className="admin-dashboard__item">
            <div className="admin-dashboard__thumbnail">
              {article.imageUrl && (
                <Image
                  src={article.imageUrl}
                  alt={article.title}
                  width={100}
                  height={150}
                  className="admin-dashboard__image"
                />
              )}
            </div>
            <div className="admin-dashboard__info">
              <p className="admin-dashboard__itemTitle">{article.title}</p>{" "}
              <p>{article.author}</p>
              {/* <span className="admin-dashboard__meta">
                — <em className="admin-dashboard__date">{new Date(article.date).toLocaleDateString()}</em>
              </span> */}
              <p className="admin-dashboard__category"> {article.category}</p>

              <div className="admin-dashboard__actions">
                <Link href={`/post/${article.slug}`} className="admin-dashboard__action admin-dashboard__action--view">
                  Voir
                </Link>

                <Link href={`/admin/edit/${article.id}`} className="admin-dashboard__action admin-dashboard__action--edit">
                  Editer
                </Link>

                <DeleteButton articleId={article.id} className="admin-dashboard__action admin-dashboard__action--delete" />
              </div>
            </div>
          </li>
        ))}
      </ul>
    </div>


    // <div className="admin-dashboard">
    //   <div className="header">
    //     <h1>Espace Admin</h1>
    //     <p>Bienvenue {session?.user?.name}</p>
    //     <Link className="new-button" href="/admin/new">
    //       ➕ Nouvel article
    //     </Link>
    //   </div>

    //   <h2>Articles existants</h2>
    //   <ul className="article-list">
    //     {articles.map((article) => (
    //       <li key={article.id} className="admin-article">
    //         <div className="article-thumbnail">
    //           {article.imageUrl && (
    //             <Image
    //               src={article.imageUrl}
    //               alt={article.title}
    //               width={400}
    //               height={600}
    //             />
    //           )}
    //         </div>

    //         <div className="article-info">
    //           <strong>{article.title}</strong> —{' '}
    //           <em>{new Date(article.date).toLocaleDateString()}</em>
    //           <p className="category">📁 {article.category}</p>

    //           <div className="admin-actions">
    //             <Link href={`/post/${article.slug}`} className="view-link">
    //               Voir
    //             </Link>
    //             <Link href={`/admin/edit/${article.id}`} className="edit-link">
    //               ✏️ Éditer
    //             </Link>
    //             <DeleteButton articleId={article.id} />
    //           </div>
    //         </div>
    //       </li>
    //     ))}
    //   </ul>
    // </div>
  );
}